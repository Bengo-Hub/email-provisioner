#!/usr/bin/env bash
# =============================================================================
# email-provisioner Build and Deploy Script
# =============================================================================
# Purpose: Builds Docker image, pushes to registry, and updates Helm values.
# Copied verbatim from notifications-api/build.sh (fully env-driven, no
# service-specific logic) per this fleet's reuse convention — only the
# APP_NAME/NAMESPACE/GIT_USER defaults below differ.
# Usage:
#   ./build.sh                     # Build only
...
# Configuration — email-provisioner owns no database (SETUP_DATABASES stays
# false), see .claude/plans/codevertex-email-hosting-service-plan.md Part 3E.
APP_NAME=${APP_NAME:-"email-provisioner"}
NAMESPACE=${NAMESPACE:-"email"}
DEPLOY=${DEPLOY:-false}
SETUP_DATABASES=${SETUP_DATABASES:-false}
ENV_SECRET_NAME=${ENV_SECRET_NAME:-"email-provisioner-env"}
...
GIT_USER=${GIT_USER:-"Email Provisioner Bot"}
...
if [[ ${DEPLOY} == "true" ]]; then
  ...
  if curl -fsSL "https://raw.githubusercontent.com/${DEVOPS_REPO}/main/scripts/tools/check-and-sync-secrets.sh" -o "$SYNC_SCRIPT" 2>/dev/null; then
    source "$SYNC_SCRIPT"
    check_and_sync_secrets "REGISTRY_USERNAME" "REGISTRY_PASSWORD" "KUBE_CONFIG" || warn "Secret sync failed - continuing with existing secrets"
    rm -f "$SYNC_SCRIPT"
  else
...
# No SETUP_DATABASES block — this service owns no database.
